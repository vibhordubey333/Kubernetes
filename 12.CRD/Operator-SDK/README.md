...
```sh
make docker-build docker-push IMG=<some-registry>/operator-sdk:tag
```
...
```sh
make deploy IMG=<some-registry>/operator-sdk:tag
```
...
```sh
make build-installer IMG=<some-registry>/operator-sdk:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/operator-sdk/<tag or branch>/dist/install.yaml
```
...
3. make generate
4. make manifests
5. make docker-build docker-push
6. export MEMCACHED_IMAGE="memcached:1.4.36-alpine"
7. make deploy
8. operator-sdk olm install --version 0.20.0
